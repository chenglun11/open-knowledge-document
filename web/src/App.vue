...
const pageCaption = computed(() => ({ dashboard: '知识同步系统的运行态势', import: '转换、校验并持久化飞书文档', documents: '统一管理开放文档模型', jobs: '追踪每一次同步与失败原因', assets: '图片和附件的下载队列', schema: '查看当前 OKD JSON Schema', audit: '关键管理操作的不可变记录', settings: '运行环境与访问控制' }[page.value]))
...
async function loadPage() {
  ...
  try {
    ...
    if (page.value === 'settings') runtime.value = await api('/api/admin/runtime')
...
async function loadNodes() {
  ...
  botBusy.value = true
  try { feishuNodes.value = await api(`/api/admin/feishu/spaces/${encodeURIComponent(selectedSpace.value)}/nodes?recursive=true&limit=500`) }
  catch (error) { ElMessage.error((error as Error).message) } finally { botBusy.value = false }
}
...
async function syncSpace() {
  ...
  botBusy.value = true
  try { const result = await api<Record<string, any>>('/api/admin/feishu/sync', { method: 'POST', body: JSON.stringify({ space_id: selectedSpace.value, recursive: true, limit: 500 }) }); ElMessage.success(`同步完成：${result.imported} 篇导入，${result.failed.length} 篇失败`) }
  catch (error) { ElMessage.error((error as Error).message) } finally { botBusy.value = false }
}
      ...
      <el-main v-loading="loading" class="main-content"><header class="page-header"><div><p class="eyebrow">CONTROL CENTER</p><h1>{{ pageTitle }}</h1><p>{{ pageCaption }}</p></div><el-button v-if="page === 'dashboard' || page === 'documents'" type="primary" :icon="UploadFilled" @click="navigate('import')">导入飞书文档</el-button></header>
        ...
        <template v-if="page === 'import'">
          <section v-if="!feishuConfig?.configured" class="panel bot-setup">
            <div class="panel-head"><div><p class="eyebrow">STEP 01</p><h3>连接飞书机器人</h3><p>凭证仅保存在服务端 /data/feishu-config.json，浏览器不会读取 App Secret。</p></div><el-tag type="warning" effect="plain">NOT CONNECTED</el-tag></div>
            <el-form label-position="top"><div class="form-two"><el-form-item label="App ID"><el-input v-model="botConfig.app_id" placeholder="cli_xxxxxxxxxx" /></el-form-item><el-form-item label="App Secret"><el-input v-model="botConfig.app_secret" type="password" show-password /></el-form-item></div><div class="form-two"><el-form-item label="平台"><el-select v-model="botConfig.brand"><el-option label="飞书（中国）" value="feishu"/><el-option label="Lark（国际）" value="lark"/></el-select></el-form-item><el-form-item label="OpenAPI Base URL（可选）"><el-input v-model="botConfig.base_url" /></el-form-item></div><div class="form-actions"><el-button type="primary" :loading="botBusy" @click="saveBotConfig">保存机器人配置</el-button></div></el-form>
          </section>
          <template v-else>
            <section class="panel source-browser">
              ...
              <div class="source-toolbar"><el-select v-model="selectedSpace" filterable placeholder="先检测连接并选择知识空间" @change="loadNodes"><el-option v-for="space in feishuSpaces" :key="space.space_id || space.id" :label="space.name || space.title || space.space_id" :value="space.space_id || space.id"/></el-select><el-button :disabled="!selectedSpace" :loading="botBusy" @click="loadNodes">刷新文档树</el-button><span>{{ feishuNodes.length }} 个节点</span></div>
              <el-table :data="feishuNodes" stripe max-height="460"><el-table-column label="知识库路径" min-width="300"><template #default="scope"><div class="title-cell"><el-icon><Document /></el-icon><div><strong>{{ scope.row.title || '未命名节点' }}</strong><small>{{ (scope.row.path || []).join(' / ') }}</small></div></div></template></el-table-column><el-table-column prop="obj_type" label="类型" width="100"/><el-table-column prop="obj_edit_time" label="飞书版本" width="140"/><el-table-column label="状态" width="110"><template #default="scope"><el-tag :type="scope.row.syncable ? 'success' : 'info'" effect="plain">{{ scope.row.syncable ? '可同步' : '跳过' }}</el-tag></template></el-table-column><el-table-column label="操作" width="110"><template #default="scope"><el-button link type="primary" :disabled="!scope.row.syncable" :loading="botBusy" @click="importNode(scope.row)">导入</el-button></template></el-table-column></el-table>
        ...
        <template v-if="page === 'settings'"><div class="settings-grid"><section class="panel"><div class="panel-head"><div><h3>运行环境</h3><p>当前服务的持久化与版本信息</p></div><el-icon class="big-icon"><Grid /></el-icon></div><dl class="settings-list"><div v-for="(value, key) in runtime" :key="key"><dt>{{ key }}</dt><dd>{{ value }}</dd></div></dl></section><section class="panel danger-zone"><h3>管理员会话</h3><p>令牌只存放在当前浏览器标签页的 sessionStorage 中，关闭标签页后自动清除。</p><el-button type="danger" plain @click="logout">退出并清除令牌</el-button></section></div></template>
      </el-main>
    </el-container>
  </el-container>
